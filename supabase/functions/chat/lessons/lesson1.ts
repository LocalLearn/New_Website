...
You can combine strings in a few different ways:
1. Using + Operator (Classic Concatenation):
 Joins strings directly. Be sure all parts are strings, or use str() to convert.
print("Hello, " + name + "!")  
2. Using f-strings (Formatted Strings):
 Allows you to embed variables inside curly braces within a string, prefixed with f.
print(f"Hello, {name}!")
This is often the most readable and modern approach.
3. Using Commas in print():
 Lets you combine different data types without explicit conversion. It adds a space between each item.
print("Hello,", name, "!")
All three methods work, but f-strings are highly recommended for clarity and flexibility.
